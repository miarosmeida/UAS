...
class RegisterActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ejk = findViewById(R.id.EditJKRegister)
    ...
    fun registerme(view: View){
        ...
        var ijk = ejk.text.toString()
        var ialamat = ealamat.text.toString()
        var cekuser = userDBHelper.cekUser(iemail)
        var status = "Gagal"
        if (cekuser =="0") {
            userDBHelper.RegisterUser(iemail, ipassword, ifullname, ijk, ialamat)
            status = "Sukses"
